<div align="center">

<p align="center">
  <img src="arnav-banner.png" width="150%"/>
</p>

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Coolvetica&weight=200&size=55&duration=2500&pause=1000&color=000000&center=true&vCenter=true&width=500&height=100&lines=Hi%2C+I+am+Arnav+Eluri" alt="Typing SVG" /></a>
### AI Engineer • ML Researcher • Full-Stack Developer
Building intelligent systems that solve real-world problems using AI, Machine Learning, and scalable software.
<br>

[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-blue?style=for-the-badge)](https://arnavsportfolio.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge\&logo=linkedin)](https://www.linkedin.com/in/arnav-eluri/)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-black?style=for-the-badge\&logo=github)](https://github.com/arnav-eluri)

</div>

---

## About Me

```yaml
Student: Computer Science
Location: Bengaluru, India

Interests:
  - Generative AI
  - Machine Learning
  - RAG Systems
  - System Design
  - Full-Stack Engineering

Current Focus:
  - LangGraph
  - Agentic AI
  - LLM Optimization
  - ML Systems
```

---

## Featured Projects



| Project            | What It Does                                            | Stack                       |
| ------------------ | ------------------------------------------------------- | --------------------------- |
| **AidGen**      | Disaster-response assistant powered by local LLMs & RAG | FastAPI • LangChain • FAISS |
| **Meal-4-All** | Converts grocery bills into AI-generated meal plans     | OCR • ML • Flask            |
| **Clarity AI**  | Extracts structured insights from online content        | NLP • FastAPI               |
| **URLSD**      | Detects phishing & malicious URLs with 95%+ accuracy    | scikit-learn • RF           |


## Technical Stack

### Languages

<p align="left">
  <img src="https://skillicons.dev/icons?i=python,ts,java,cpp,sql" />
</p>

---

### Generative AI

<p align="left">
  <img src="https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white"/>
  <img src="https://img.shields.io/badge/Gemini-4285F4?style=for-the-badge&logo=google&logoColor=white"/>
  <img src="https://img.shields.io/badge/Claude-D97757?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/OpenRouter-000000?style=for-the-badge"/>
</p>

`RAG` • `Prompt Engineering` • `Multi-Agent Systems` • `Semantic Search` • `Embeddings`

---

### AI / Machine Learning

<p align="left">
  <img src="https://skillicons.dev/icons?i=pytorch,tensorflow" />
</p>

`scikit-learn` • `OpenCV` • `Pandas` • `NumPy` • `MLflow`

---

### Backend

<p align="left">
  <img src="https://skillicons.dev/icons?i=fastapi,flask,nodejs,postgres" />
</p>

`REST APIs` • `JSON Workflows` • `Async Programming`

---

### Frontend

<p align="left">
  <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind" />
</p>

---

### DevOps & Tools

<p align="left">
  <img src="https://skillicons.dev/icons?i=docker,git,github,linux,vscode,postman" />
</p>

---

### Currently Exploring

`LangGraph` • `Agentic AI` • `MCP` • `LLM Evaluation` • `AI Engineering`

---


## Currently Learning

`LangGraph` • `Agentic AI` • `MCP` • `LLM Evaluation` • `Distributed Systems`

---

<div align="center">

### Research • Engineer • Ship

*Crafting solutions, not just code.*

</div>
